...
{
    public class Arrivals : IAirlineFlights
    {
        ...
        public void ShowFlights(List<Flights> arrivalsFlights)
        {
            Console.SetCursorPosition(0, 7);
            for (int i = 0; i < arrivalsFlights.Count; i++)
            {
                if (arrivalsFlights[i].DataFlight != null)
                {
                    Console.CursorLeft = 38;
                    Console.Write($"{i + 1}: ");
                    Console.CursorLeft = 45;
                    DateTime date = (DateTime)arrivalsFlights[i].DataFlight;
                    Console.Write(date.ToString("HH:mm dd/MM/yyyy"));
                    Console.CursorLeft = 70;
                    Console.Write(arrivalsFlights[i].Flight);
                    Console.CursorLeft = 89;
                    Console.Write(arrivalsFlights[i].City);
                    Console.CursorLeft = 113;
                    Console.Write(arrivalsFlights[i].Terminal);
                    Console.CursorLeft = 121;
                    Console.Write(arrivalsFlights[i].Gate);
                    Console.CursorLeft = 140;
                    Console.Write(arrivalsFlights[i].Status);
                    Console.CursorLeft = 150;
                    Console.Write($"${arrivalsFlights[i].Price}");
                    Console.WriteLine();
                    Console.WriteLine();
                }
            }
        ...
        public void AddFlight(ref List<Flights> arrivalsFlights)
        {
            DataStrings dataStr = new DataStrings();
            ...
            arrivalsFlights.Add(new Flights()
            {
